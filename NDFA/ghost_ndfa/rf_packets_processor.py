...
class RfPacketsProcessor:
    ...
    def __init__(self, config, communicator, interface, conn, ps_cur, lock):
        ...
        self.pcap_path_rf = interface.config.pcap_path
        ...
        self.rf_batches_kill_time = float(
            config.get('Time', 'rf_batches_kill_time'))
    ...
    def setup(self):
        ...
        if self.clear_db_on_startup:
            self.execute_on_db("TRUNCATE TABLE rf869_packets RESTART IDENTITY;TRUNCATE TABLE rf869_flows RESTART IDENTITY;")
    ...
    def process_pcap_file(self, path, profiling=False):
        script = os.path.dirname(os.path.realpath(__file__)) + "/pcap_rf_process.py"
        try:
            a = check_output(["python", script, path]) # e.g, "python pcap_rf_process.py XXX.pcap"
            ...
            for pd in packets:
                ...
                ###############################################
                if pd['address'] not in self.device_set:
					self.device_set.add(pd['address'])
					if_id = InterfaceId()
					...
					if  (current_time-self.start_time)> self.silence_alert: #if the silence alert period exceed the current time
						self.publish_new_device(if_id, pd['address'], current_time)
				################################################
                ...
                self.execute_on_db(
                    "INSERT INTO rf869_packets (created, data) values(now(), {});".format(Json(pd)))
        except:
            logging.error("Failed to process RF pcap file : {}".format(path))
  
    def get_batches_key(self,p):
            return p["address"]  # it is only one address in the packet.
    ...
    def update_batches(self, p):
            ...
            elif p["time"] - self.running_batch[key][-1]["time"] < self.rf_batches_kill_time:
            ...
            for key in self.running_batch.keys():
                if self.running_batch[key][-1]['time']< p['time'] - self.rf_batches_kill_time:
                        self.kill_batch(key)
    ...
    def kill_batch(self, key):
        ...
        batch_details["address"] = self.running_batch[key][0]["address"]
        ...
        batch_id = self.execute_on_db("INSERT INTO rf869_flows (created, data) values(now(), {}) RETURNING id;".format(Json(batch_details)),get_results=True)
        # added for sending zmq message for each flow
        ###############################################
        if_id = InterfaceId()
        if_id.value = self._interface.id.value
        self.publish_new_flow(if_id, batch_id,batch_details["address"], sum_size, batch_details["number_of_packets"] )
        ################################################
    ...
    def run(self, loop):
        ...
        #fetch the RF address of from device_info Table
        self.lock.acquire()
        self.ps_cur.execute("SELECT tves_rf869 from device_info where interface_type = '2';")
        rf_addresses = self.ps_cur.fetchall()
        self.lock.release()
        for rf_address in rf_addresses:
                s= '{0:#0{1}x}'.format(rf_address[0],8)
                s = s[2:]
                s = unicode(s, "utf-8")
                self.device_set.add(s)
                #print  self.device_set
        
        if self.pcap_path_rf[-5:] == ".pcap":  # file_mode
            if loop:
                while self.keep_running:
                    self.process_pcap_file(self.pcap_path_rf)
            else:
                self.process_pcap_file(self.pcap_path_rf)
            ...
            if loop:
                while self.keep_running:
                    ...
                    common_files = 0
                    new_files = 0
                    for filename in glob.iglob(self.pcap_path_rf + "/*.pcap"):
    ...
    def publish_new_flow(self, if_id, flow_id, address, total_bytes, num_packets):
        '''
        Publishes an alert message to GHOST's modules on the progress of
        processing the PCAP files.
        '''
        ...
        fl.flow_id = flow_id
        fl.src_address = address
        fl.dst_address = "" #empty field (implied gateway)
        fl.total_bytes_a = total_bytes
        fl.packets_a = num_packets
        fl.total_bytes_b = -1
        fl.packets_b = -1
        fl.type = 2 #  InterfaceType:     TVES_RF869 = 2;
    ...
    def publish_new_device(self, if_id, addr, event_time):
        ...
        Parameters:
			 ...
			 InterfaceType -- type of the interface (TVES_RF869)
        ...
        alert.if_id.value  = if_id.value
        alert.address  = addr
        alert.type = 2 #  InterfaceType:   TVES_RF869 = 2;
