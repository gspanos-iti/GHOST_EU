...
class ZbPacketsProcessor:
    ...
    def __init__(self, config, communicator, interface, conn, ps_cur, lock):
        ...
        self.pcap_path_zb = interface.config.pcap_path
        ...
        self.zb_batches_kill_time = float(
            config.get('Time', 'zb_batches_kill_time'))
    ...
    def setup(self):
        ...
        if self.clear_db_on_startup:
            self.execute_on_db("TRUNCATE TABLE zgb_packets RESTART IDENTITY;TRUNCATE TABLE zgb_flows RESTART IDENTITY;")
    ...
    def process_pcap_file(self, path, profiling=False):
        script = os.path.dirname(os.path.realpath(__file__)) + "/pcap_zb_process.py"
        try:
                a = check_output(["python", script, path]) # e.g, "python pcap_zb_process.py XXX.pcap"
                ...
                for pd in packets:
                        ...
                        ###############################################
                        if pd['dst_zb_addr'] not in self.device_set:
							self.device_set.add(pd['dst_zb_addr'])
							if_id = InterfaceId()
							...
							if  (current_time-self.start_time)> self.silence_alert: #if the silence alert period exceed the current time
								self.publish_new_device(if_id, pd['dst_zb_addr'], current_time)
                        if pd['src_zb_addr'] not in self.device_set:
							self.device_set.add(pd['src_zb_addr'])
							if_id = InterfaceId()
							if_id.value = self._interface.id.value
							if  (current_time-self.start_time)> self.silence_alert: #if the silence alert period exceed the current time
								self.publish_new_device(if_id, pd['src_zb_addr'], current_time)
				################################################
				
				
                        self.update_batches(pd)
                        self.execute_on_db(
                        "INSERT INTO zgb_packets  (created, data) values(now(), {});".format(Json(pd)))
        except Exception as error:
                logging.error("Failed to process ZGB pcap file : {}".format(path))


    def get_batches_key(self,p):
        if p['src_zb_addr']<p['dst_zb_addr']:
            return p['src_zb_addr']+p['dst_zb_addr']
        else:
            return p['dst_zb_addr']+ p['src_zb_addr']

    def update_batches(self, p):

        key = self.get_batches_key(p)
        if key not in self.running_batches.keys() : # No existing batch -> start a new one
            self.running_batches[key] = [p]
        elif p["time"] - self.running_batches[key][-1]["time"] < self.zb_batches_kill_time:
            self.running_batches[key].append(p)
        else:
            self.kill_batch(key)
            self.running_batches[key] = [p]

        ## We have to manually check all running batches, in order to kill expired ones
        for key in self.running_batches.keys():
            if self.running_batches[key][-1]['time']< p['time'] - self.zb_batches_kill_time:
                self.kill_batch(key)
                del self.running_batches[key]

    def kill_batch(self,key):
        batch_details = {}


        batch_details["start_time"] = self.running_batches[key][0]["time"]
        batch_details["stop_time"] = self.running_batches[key][-1]["time"]
        batch_details["duration"] = batch_details["stop_time"] - batch_details["start_time"]
        batch_details["number_of_packets"] = len(self.running_batches[key])
        batch_details["src_zb_addr"] = self.running_batches[key][0]["src_zb_addr"]
        batch_details["dst_zb_addr"] = self.running_batches[key][0]["dst_zb_addr"]
        min_size = self.running_batches[key][0]["length"]
        max_size = self.running_batches[key][0]["length"]
        sum_size = 0
        total_bytes_a = 0
        total_bytes_b = 0
        packets_a = 0
        ...
        for p in self.running_batches[key]:
            if p["length"] > max_size:
                max_size = p["length"]
            if p["length"] < min_size:
                min_size = p["length"]
            if p["src_zb_addr"]  == batch_details["src_zb_addr"]: # if the sender is A
                total_bytes_a = total_bytes_a +  p["length"] 
                packets_a = packets_a + 1
            elif p["src_zb_addr"]  == batch_details["dst_zb_addr"] : # if the sender is B
        ...
        batch_id = self.execute_on_db("INSERT INTO zgb_flows (created, data) values(now(), {}) RETURNING id;".format(Json(batch_details)),get_results=True)
        #print("--->>>",batch_id)	
        # added for sending zmq message for each flow
        ###############################################
        if_id = InterfaceId()
        if_id.value = self._interface.id.value
        self.publish_new_flow(if_id, batch_id,batch_details["src_zb_addr"] , batch_details["dst_zb_addr"], total_bytes_a, packets_a, total_bytes_b, packets_b)
 		################################################
    ...
    def run(self, loop):
        ...
        self.device_set.add(u'0x00000000') #address of ZB controller
		#fetch the ZB address of from device_info Table	
        self.lock.acquire()		
        self.ps_cur.execute("SELECT zig_bee_device from device_info where interface_type = '3';")
        zb_addresses = self.ps_cur.fetchall()
        self.lock.release()
        for zb_address in zb_addresses:
                s= '{0:#0{1}x}'.format(zb_address[0],10)
                s = unicode(s, "utf-8")
                self.device_set.add(s)
                #print  (self.device_set)

        if self.pcap_path_zb[-5:] == ".pcap":  # file_mode
            if loop:
                while self.keep_running:
                    self.process_pcap_file(self.pcap_path_zb)
            else:
                self.process_pcap_file(self.pcap_path_zb)
            ...
            if loop:
                while self.keep_running:
                    ...
                    common_files = 0
                    new_files = 0
                    for filename in glob.iglob(self.pcap_path_zb + "/*.pcap"):
    ...
    def publish_new_flow(self, if_id, flow_id, src_address, dst_address, total_bytes_a, packets_a, total_bytes_b, packets_b):
        ...
        fl.type = 3 #  InterfaceType:   ZIGBEE = 3;
        # Publish it
    ...
    def publish_new_device(self, if_id, addr, event_time):
        ...
        Parameters:
			 ...
			 InterfaceType -- type of the interface (ZIGBEE)
        ...
        alert.if_id.value  = if_id.value
        alert.address  = addr
        alert.type = 3 #  InterfaceType:   ZIGBEE = 3;
