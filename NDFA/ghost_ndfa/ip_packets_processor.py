...
class IpPacketsProcessor:
    ...
    def __init__(self, config, communicator, interface, conn, ps_cur, lock):
        ...
        self.hosting_GW = config.get('Hosting', 'gateway') 
    ...
    def setup(self):
        ...
        #    "INSERT INTO ipv4_packets (created, data) values($1, $2)")

        if self.clear_db_on_startup:
            self.ps_cur.execute("TRUNCATE TABLE ipv4_packets RESTART IDENTITY;TRUNCATE TABLE ipv4_flows RESTART IDENTITY;")
        self.conn.commit()
        self.lock.release()
    ...
    def extract_flows(self):
        ...
        # while not os.path.isfile(self.eth0_global_pcap_path):
        #     time.sleep(0.1)
        #     print("waiting for global pcap file to be written")
        ...
        for f in flows_data_text.split('\n'):
            ...
            if len(flow_data) == 16:
                ...
                try:
                    ...
                    flow['mac_a'] = self.macs_dict[flow['ip_a']][0]
                except:
                    flow['mac_a'] = ''
                try:
                    # Read MAC for dictionary
                    flow['mac_b'] = self.macs_dict[flow['ip_b']][0]
                except:
                    flow['mac_b'] = ''
    ...
    def trim_flows_2(self, cur_flows, cap_end_time):
        if self.storage_db == "psql":
            self.lock.acquire()
            self.ps_cur.execute("SELECT * FROM ipv4_flows WHERE data->>'status'='alive';")
                ...
                for of in old_flows:
                    if cf['ip_a'] == of[1]['ip_a'] and cf['ip_b'] == of[1]['ip_b'] and cf['port_a'] == of[1]['port_a'] and cf['port_b'] == of[1]['port_b']:
                        ...
                        if cf['packets_a']+cf['packets_b'] > of[1]['packets_a'] + of[1]['packets_b']:
                            ...
                            cf['status'] = 'alive'
                            self.lock.acquire()
                            self.ps_cur.execute("UPDATE ipv4_flows SET data={} WHERE id = {};".format(Json(cf), of[0]))
                            self.conn.commit()
                            self.lock.release()
                        break
                if not found:
                    cf['status'] = 'alive'
                    self.lock.acquire()
                    self.ps_cur.execute("INSERT INTO ipv4_flows (created, data) values(?, ?);", (datetime.utcnow(), Json(cf)))
                    self.conn.commit()
                    self.lock.release()
                    # print("flow added")
            ...
            for of in old_flows:
                if of[1]['ts_end'] < max_ts_end - self.kill_flow_waitime:
                    of[1]['status'] = 'expired'
                    self.lock.acquire()
                    self.ps_cur.execute("UPDATE ipv4_flows SET data={} WHERE id = {};".format(Json(of[1]), of[0]))
                    self.conn.commit()
                    self.lock.release()
    ...
    def trim_flows_in_mem(self, cur_flows, cap_end_time):
        ...
        for i, of in enumerate(self.active_flows):
            if of['ts_end'] < max_ts_end - self.kill_flow_waitime:
                ...
                self.lock.acquire()
                self.ps_cur.execute("INSERT INTO ipv4_flows (created, data) values(now(), {}) RETURNING id;".format(Json(of)))
    ...
    def flush_flows(self, cur_flows):
        ...
        for f in cur_flows:
            if self.storage_db == "psql":
                self.ps_cur.execute("INSERT INTO ipv4_flows (created, data) values(now(), {});".format( Json(f)))
                self.conn.commit()
        self.lock.release()
    ...
    def read_pcap_dpkt(self, cap, cap_length, start_index=0):
        ...
        for ts, buf in cap:
            ...
            try:
                ...
                pd = self.packet_check_dpkt(p, ts)  #, previous_hash)
                # pd['payload'] = payloads[i - start_index]
                # pd['id'] = existing_packets + i - start_index
                if pd and self.storage_db == "psql":
                    sql = sql + "INSERT INTO ipv4_packets (created, data) values(now(), {});".format(Json(pd))
    ...
    def packet_check_dpkt(self, eth, ts):
        packet_details = {}
        ...
        try:
            ...
            eth_src = ':'.join('%02x' % compat_ord(b) for b in eth.src)
            eth_dst = ':'.join('%02x' % compat_ord(b) for b in eth.dst)
            #check if it is internal or external
            # send ZMQ to RE about new IP
            descr = ''
            current_time = time.time()
            if (ip_src not in self.macs_dict):
				...
				self.macs_dict[ip_src] = [eth_src, descr]
            elif (self.macs_dict [ip_src] == ''):   #means that this IP is inserted from device_info table, but do not know the MAC.
				if self.is_internal (ip_src):
					descr = 'internal'
				else:
					descr = 'external'
            ...
            if (ip_dst not in self.macs_dict) :
                ...
                self.macs_dict[ip_dst] = [eth_dst, descr]
            elif  (self.macs_dict [ip_dst] == ''):  #means that this IP is inserted from device_info table, but do not know the MAC.
				if self.is_internal (ip_dst):
					descr = 'internal'
				else:
					descr = 'external'
				self.macs_dict[ip_dst] = [eth_dst, descr]
            packet_details['length'] = ip.len #length of IP frame
            packet_details['id'] = ip.id  #id of IP frame
            packet_details['src_mac'] = eth_src #mac addresses
            packet_details['dst_mac'] = eth_dst
            packet_details['src_ip'] = ip_src  #IP addresses
            packet_details['dst_ip'] = ip_dst
            packet_details['time'] = float(ts)
            
            if  ip.p == dpkt.ip.IP_PROTO_TCP : 		#transport layers
                packet_details['transport'] = "TCP"
                packet_details['flags'] = ip.data.flags
                packet_details['options'] = repr(ip.data.opts)
                packet_details['sport'] = int(ip.data.sport)  #ports
                packet_details['dport'] = int(ip.data.dport)
            elif   ip.p == dpkt.ip.IP_PROTO_UDP :
                packet_details['transport'] = "UDP"
                packet_details['sport'] = int(ip.data.sport)  #ports
                packet_details['dport'] = int(ip.data.dport)
            else:
                packet_details['transport'] = ip.p 			#different than UDP or TCP
            packet_details['data'] = repr(ip.data.data)
    ...
    def publish_new_flow(self, if_id, flow_id, src_address, dst_address, total_bytes_a, packets_a, total_bytes_b, packets_b):
		...
		Parameters:
          src_address -  IP address of A node
          dst_address - string -  IP address of B node
          total_bytes_a - the total bytes from A to B
          packets_a - the number of packets from A to B. 
          total_bytes_b - the total bytes from B to A.
          packets_b - the number of packets from B to A.
        '''
        # Build the Alert message
        fl = FlowAdded()
    ...
    def run(self, loop):
        ...
        if (self.hosting_GW== 'TVES'):
                self.macs_dict[self.get_ip_address('eth0:1')] = [self.get_MAC_address('eth0:1'), 'GW']  # public IP interface of GW
                self.macs_dict[self.get_ip_address('wlan0')] = [self.get_MAC_address('wlan0'), 'GW'] # wireless IP interface of GW 
                self.macs_dict[self.get_ip_address('ppp0')] = [self.get_MAC_address('ppp0'), 'GW'] # PPP IP interface of GW 
                #internal IP ranges
                self.internal_IP_range = [self.get_ip_address('wlan0')+'/24', self.get_ip_address('eth0:1')+'/24'] #Internal Range of  IPs     
        elif (self.hosting_GW== 'KALOS'):
                self.macs_dict[self.get_ip_address('eth0')] = [self.get_MAC_address('eth0'), 'GW']  # public IP interface of GW
                self.macs_dict[self.get_ip_address('wlan0')] = [self.get_MAC_address('wlan0'), 'GW'] # wireless IP interface of GW 
                #internal IP ranges
                self.internal_IP_range = [self.get_ip_address('wlan0')+'/24', self.get_ip_address('eth0')+'/24'] #Internal Range of  IPs     

        
		#broadcast IP for wireless network
        broadcast  = self.get_ip_address('wlan0').split('.')
        broadcast[-1] = 255
        broadcast = '.'.join(str(n) for n in broadcast)
        self.macs_dict[broadcast] = '' # broadcast wireless IP address of GW 

        #self.macs_dict[self.get_ip_address('ens33')] = [self.get_MAC_address('ens33'), 'GW']  # public IP interface of GW
        #self.internal_IP_range = ['192.168.1.1'+'/24'] #Internal Range of  IPs     
        #fetch the ip address of from device_info Table
        self.lock.acquire()
        self.ps_cur.execute("SELECT ip4_device from device_info where interface_type = '0';")
        ip_addresses = self.ps_cur.fetchall()
        self.lock.release()
        for ip_addr in ip_addresses:
                self.macs_dict[socket.inet_ntoa(struct.pack(">L", ip_addr[0]))] = "" #we dont know the MAC address yet
